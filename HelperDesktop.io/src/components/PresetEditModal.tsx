...
export default function PresetEditModal({ preset, onClose, onSave }: PresetEditModalProps) {
  ...
  return (
    <Modal onClose={onClose} size="md">
      <ModalClose onClick={onClose} disabled={saving} />
      <ModalHeader
        icon={<span style={{ fontSize: 22 }}>{icon || '🎮'}</span>}
        title={preset ? 'Редактировать пресет' : 'Новый пресет'}
        subtitle="Приложения, которые откроются при запуске"
      />
      ...
      <div className="preset-form">
        ...
        <div className="preset-apps-list">
          {apps.map((app, idx) => (
            <div className="preset-app-row" key={idx}>
        ...
        <div className="preset-modal-actions">
          <button className="settings-btn settings-btn-ghost" onClick={onClose} disabled={saving}>
            Отмена
          </button>
          <button className="modal-submit" onClick={handleSave} disabled={saving || !name.trim()}>
