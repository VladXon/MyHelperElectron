...
| Задача | Описание |
|--------|----------|
| Добавить CSP заголовки | Защита от XSS |
| Добавить path traversal protection | Валидация имён файлов |

### P1 (1-2 месяца)

| Задача | Описание |
|--------|----------|
| Drizzle ORM вместо raw SQL | Типобезопасность, миграции |
| Интеграционные тесты | Vitest + реальная SQLite |
...
| Задача | Описание |
|--------|----------|
| Feature-based архитектура | Изоляция фич в renderer |
| AI Assistant (LM Studio) | Локальные LLM модели |
...
### Фаза 2: Инфраструктура (2-3 месяца)
...
| `routes/*.test.ts` | API routes integration |
