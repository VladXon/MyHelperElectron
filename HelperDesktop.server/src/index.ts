...
app.use((req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    if (code >= 500) {
      logger.error(line, { method: req.method, url: req.originalUrl, status: code, duration: ms, ip, requestId, body: maskPassword(req.body) });
    } else if (code >= 400) {
...
app.get('/api/health', (_req, res) => {
  try {
    ...
    logger.error('Health check failed', { error: (e as Error).message });
...
botChild.stderr?.on('data', (data) => {
  const output = data.toString().trim();
  if (output) logger.error(`[BOT] ${output}`);
});

botChild.on('error', (err) => logger.error('Failed to start bot', { error: err.message }));
